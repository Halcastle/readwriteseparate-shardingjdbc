...
//        ComplexKeysShardingValue:ComplexKeysShardingValue(logicTableName=shopping_customer_info, columnNameAndShardingValuesMap={certid=[0]}, columnNameAndRangeValuesMap={})
        ...
        Set<Map.Entry<String,String>> columnNameAndShardingValuesSet = columnNameAndShardingValuesMap.entrySet();
//        String[] splitdbshardings1 = null;
//        availableTargetNames.stream().collect(Collectors.toList()).;
//        String[] splitdbshardings = dbshardings.split(",");
//        for(String dbsharding:splitdbshardings){
//            for(Map.Entry columnNameAndShardingValue : columnNameAndShardingValuesSet){
//                if(dbsharding.equals(columnNameAndShardingValue.getKey())){
        BigInteger value = null;
//        value = (BigInteger)columnNameAndShardingValuesMap.get("certid");
        ArrayList certid = (ArrayList) columnNameAndShardingValuesMap.get("certid");
        value = new BigInteger((String)certid.get(0));
//        BigInteger value = (BigInteger) columnNameAndShardingValue.getValue();
        if(null != value){
...
//        value = (BigInteger)columnNameAndShardingValuesMap.get("id");
        ArrayList id = (ArrayList) columnNameAndShardingValuesMap.get("id");
        value = new BigInteger((String)certid.get(0));
        BigInteger[] bigIntegers = value.divideAndRemainder(BigInteger.valueOf(datasourcelength));
        collection.add("ds"+bigIntegers[1].toString());
//                }
